...
import { getOrder } from "../../services/callAPI";
import { Card } from "react-native-elements";
import { useIsFocused } from "@react-navigation/native";
import API from "../../services/api";
import FontAwesome from "react-native-vector-icons/FontAwesome";
export default function CLOrderConfirmScreen({ navigation, route }) {
      ...
      <ScrollView>
        {ConfirmingOrders.map((item) => (
          <SafeAreaView key={item.id}>
            <TouchableOpacity
              onPress={() => {
                navigation.navigate("SVOrderDetail", {
                  orders: item,
                  userData: userData,
                  userInfo: userInfo,
                });
              ...
              <Card containerStyle={{ backgroundColor: "#00FF00" }}>
                ...
                <View style={{ alignItems: "flex-end" }}>
                  <TouchableOpacity
                    onPress={() => {
                      API.post(
                        `order/update/${item.id}`,
                        { isCanceled: 1, deliveryAt: new Date() },
                        {
                          headers: {
